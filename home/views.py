...
def admin_panel(request):
    try:
        ...
        if user.email == 'ssshahriar17@gmail.com':
...
from .models import UserFeedback, UserModel
...
def feedback(request):
    try:
        ...
        # If the request is a POST, save the user's feedback
        if request.method == 'POST':
            if request.POST.get('feedbackMsg'):
                ...
                return redirect('/feedback')  # Redirect to feedback page to display the feedback again

        # If it's a GET request, fetch all the feedback to display
        feedback_list = UserFeedback.objects.all().order_by('-id')  # Retrieve feedback in descending order

        return render(request, 'feedback.html', {'user': user, 'feedback_list': feedback_list})

    except UserModel.DoesNotExist:
        # If user is not found in session, prompt login
        messages.error(request, 'You need to login first')
        return redirect('authenticate')
...
def claim_owner(request, token):
    ...
    # Execute the query to fetch the post details
    cursor.execute(
        'SELECT * FROM app_users au, user_posts up WHERE au.id = up.publisherId and up.id = %s', [token])
    posts = cursor.fetchall()
    cursor.close()
    ...
    if not posts:
        messages.error(request, "Post not found or invalid token.")
        return redirect('/')

    try:
        # Log session email for debugging
        if 'email' not in request.session:
            messages.error(request, "User session has expired. Please log in again.")
            return redirect('/login/')
        ...
        if user.completeProfile == '100%':
            ...
            # Populate basic claim details
            claimOwner.claimerId = user.id
            claimOwner.claimerName = user.name
            claimOwner.claimerEmail = user.email
            claimOwner.postId = token
            
            # Fetching data from the result (ensure the indices are correct)
            if len(posts[0]) >= 19:  # Ensures there are at least 19 columns
                claimOwner.postPunlisherEmail = posts[0][2]
                claimOwner.postPunlisherName = posts[0][18]
            else:
                messages.error(request, "Unexpected post structure, please contact support.")
                return redirect('/')

            # Check if a clue or image is provided to auto-confirm the claim
            print(f"POST data: {request.POST}")
            print(f"FILES data: {request.FILES}")

            if len(request.FILES) != 0:  # Check if a file is provided
                claimOwner.claimFileImg = request.FILES['secretPic']
                claimOwner.status = 'Confirmed'  # Auto-confirm if an image is provided
                print("Image provided. Claim confirmed.")
            elif 'clue' in request.POST and request.POST['clue'].strip() != '':  # Check if a clue is provided
                claimOwner.clue = request.POST['clue'].strip()
                claimOwner.status = 'Confirmed'  # Auto-confirm if a clue is provided
                print("Clue provided. Claim confirmed.")
            else:
                claimOwner.status = 'Pending'  # Default to pending if no clue or image
                print("No clue or image provided. Claim pending.")

            # Save the claim
            claimOwner.save()

            # Deduct points from user for making the claim
            ...
            # Inform user to complete their profile
            messages.error(request, "Complete your profile first!")
            return render(request, 'view_profile.html', {'user': user})

    except Exception as e:
        # Log the error (optional) and display a generic error message
        print(f"Error during claim submission: {e}")
        messages.error(
